...
import difftool.util.dialog.GitCompareDialog;
import difftool.util.model.GitLogData;



public class CompareWithHashIdHandler extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		GitLogData gitLogData = GitHandlerUtil.getGitLogs(resource);
		GitCompareDialog dialog = new GitCompareDialog(window.getShell(),gitLogData);
		dialog.create();
		//MyTitleAreaDialog.setDialogHelpAvailable(false);
		if (dialog.open() == Window.OK) {
		    System.out.println(dialog.getCommitHashId());
		    System.out.println(dialog.getLastName());
		    GitHandlerUtil.compareWithHashId(resource, dialog.getCommitHashId());
		}
		return null;
	}
